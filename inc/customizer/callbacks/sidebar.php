<?php
/**
 * Sidebar render callbacks.
 *
 * Shared by template-parts/sidebar.php and the Customizer partial.
 *
 * @package Portfolio
 */

defined( 'ABSPATH' ) || exit;

/**
 * The profile image URL, or the packaged placeholder when none is set.
 *
 * @return string
 */
function portfolio_profile_image_src() {
	$image = get_theme_mod( 'portfolio_sidebar_profile_image', '' );

	return $image ? $image : get_template_directory_uri() . '/assets/img/profile.svg';
}

/**
 * Render the profile image, filling whatever box it is placed in.
 *
 * Rendered twice on the front page: once in the sidebar column and once in the
 * hero, since the sidebar is a floating icon pill below the nav breakpoint and
 * has no room for a photo. Only one is ever visible. The id is therefore
 * optional â€” it is the Customizer partial's selector, so only the sidebar copy
 * carries it and the document keeps a single #sidebar-profile.
 *
 * @param string $id Element id, or '' for none.
 * @return string
 */
function portfolio_render_sidebar_profile( $id = 'sidebar-profile' ) {
	$src = portfolio_profile_image_src();

	ob_start();
	?>
	<div <?php echo $id ? 'id="' . esc_attr( $id ) . '" ' : ''; ?>class="absolute inset-0 rounded-full overflow-hidden border-4 border-white shadow-lg ring-2 ring-[#004fff]/30">
		<img
			alt="<?php esc_attr_e( 'Profile picture', 'portfolio' ); ?>"
			class="w-full h-full object-cover"
			src="<?php echo esc_url( $src ); ?>"
		>
	</div>
	<?php
	return ob_get_clean();
}

/**
 * Render the sidebar's social links.
 *
 * The two URLs are Customizer settings (Customizer > Sidebar) rather than
 * markup: both icons used to be hard-coded to href="#", which is a link that
 * looks live and goes nowhere. An unset URL now drops its icon entirely, so
 * the row shows only the profiles that actually exist — and the whole row
 * disappears when neither is set, instead of leaving an empty gap at the foot
 * of the column.
 *
 * @return string
 */
function portfolio_render_sidebar_social() {
	$links = array(
		'linkedin' => array(
			'url'   => get_theme_mod( 'portfolio_sidebar_linkedin', '' ),
			'label' => __( 'LinkedIn', 'portfolio' ),
			'class' => 'hover:text-[#0A66C2]',
			'path'  => 'M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z',
		),
		'github'   => array(
			'url'   => get_theme_mod( 'portfolio_sidebar_github', '' ),
			'label' => __( 'GitHub', 'portfolio' ),
			'class' => 'hover:text-[#181717]',
			'path'  => 'M12 .297c-6.63 0-12 5.373-12 12 0 5.303 3.438 9.8 8.205 11.385.6.113.82-.258.82-.577 0-.285-.01-1.04-.015-2.04-3.338.724-4.042-1.61-4.042-1.61C4.422 18.07 3.633 17.7 3.633 17.7c-1.087-.744.084-.729.084-.729 1.205.084 1.838 1.236 1.838 1.236 1.07 1.835 2.809 1.305 3.495.998.108-.776.417-1.305.76-1.605-2.665-.3-5.466-1.332-5.466-5.93 0-1.31.465-2.38 1.235-3.22-.135-.303-.54-1.523.105-3.176 0 0 1.005-.322 3.3 1.23.96-.267 1.98-.399 3-.405 1.02.006 2.04.138 3 .405 2.28-1.552 3.285-1.23 3.285-1.23.645 1.653.24 2.873.12 3.176.765.84 1.23 1.91 1.23 3.22 0 4.61-2.805 5.625-5.475 5.92.42.36.81 1.096.81 2.22 0 1.606-.015 2.896-.015 3.286 0 .315.21.69.825.57C20.565 22.092 24 17.592 24 12.297c0-6.627-5.373-12-12-12',
		),
	);

	ob_start();
	?>
	<div id="sidebar-social" class="sidebar-social">
		<?php foreach ( $links as $link ) : ?>
			<?php if ( '' === $link['url'] ) : ?>
				<?php continue; ?>
			<?php endif; ?>
			<a class="<?php echo esc_attr( $link['class'] ); ?> transition-colors"
				href="<?php echo esc_url( $link['url'] ); ?>"
				target="_blank" rel="noopener noreferrer"
				aria-label="<?php echo esc_attr( $link['label'] ); ?>">
				<svg class="w-8 h-8" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true" focusable="false"><path d="<?php echo esc_attr( $link['path'] ); ?>"></path></svg>
			</a>
		<?php endforeach; ?>
	</div>
	<?php
	return ob_get_clean();
}
