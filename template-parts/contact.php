...
<aside class="contact-panel" role="dialog" aria-modal="true" aria-label="<?php esc_attr_e( 'Contact', 'portfolio' ); ?>" aria-hidden="true">
	...
	<button type="button" class="cp-back" data-contact-close>
		<span class="cp-back-ico" aria-hidden="true">
			<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="butt" stroke-linejoin="round"><path d="M24 12H8M15 19l-7-7 7-7"></path></svg>
		</span>
	...
	<div class="cp-num">
		...
		<?php endif; ?>
		<?php
		$cp_phone_primary = $cp_phone ? $cp_phone : '9803280069';
		$cp_phone_wa      = apply_filters( 'portfolio_contact_phone_whatsapp', '9868843699' );
		$cp_wa_digits     = apply_filters( 'portfolio_whatsapp_number', '9779868843699' );
		$cp_wa_url        = 'https://wa.me/' . preg_replace( '/[^0-9]/', '', $cp_wa_digits );
		?>
		<h3 class="cp-phones">
			<a href="tel:<?php echo esc_attr( preg_replace( '/[^0-9+]/', '', $cp_phone_primary ) ); ?>"><?php echo esc_html( $cp_phone_primary ); ?></a>
			<span class="cp-phones-sep"> / </span>
			<a href="tel:<?php echo esc_attr( preg_replace( '/[^0-9+]/', '', $cp_phone_wa ) ); ?>"><?php echo esc_html( $cp_phone_wa ); ?></a>
			<span class="cp-phones-sep"> (</span>
			<a
				class="cp-wa"
				href="<?php echo esc_url( $cp_wa_url ); ?>"
				target="_blank"
				rel="noopener noreferrer"
				aria-label="<?php esc_attr_e( 'Chat on WhatsApp', 'portfolio' ); ?>"
			>
